...
La production est engagée case par case. Une image validée devient une référence obligatoire de continuité. Aucun passage automatique à la case suivante.

> État Git des binaires : les PNG générés dans la session ChatGPT ne sont pas encore présents dans le dépôt. Le connecteur GitHub actif permet les écritures texte/Git Data mais ne prend pas directement un fichier PNG local comme paramètre d'upload. Les statuts ci-dessous reflètent les validations humaines réelles ; ils ne doivent pas être interprétés comme une confirmation de présence du fichier binaire dans `BD_pdf/images/`.

| Cases | Chemin canonique prévu | Statut actuel | Continuité |
|---|---|---|---|
...
- Léa est représentée comme une enfant de **12 ans**, en style manga cinématographique cohérent.
- Le pont est un tablier de planches **tenu/suspendu par des cordes**, sans structure rigide ajoutée comme support du tablier.
...
Les validations humaines/3D listées à la fin de `04_STORYBOARD_DETAILLE.md` restent ouvertes : géométrie caverne, mécanique exacte du mousqueton, chorégraphie de fermeture, worldline de l'anneau et références visuelles de production.