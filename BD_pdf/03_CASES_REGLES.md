# LA BOUCLE — Règles strictes de découpage case par case
...
- aucune narration explicative inventée ;
- aucune pensée intérieure inventée ;
- aucun objet important inventé ;
- aucun geste causal inventé ;
- aucune modification d'heure ;
...
## Procédure obligatoire avant CHAQUE case

Avant de générer une image, préparer une fiche de case avec exactement les rubriques suivantes.
...
**État des objets :** pont, mousqueton, anneau, montre, pierre, feuille, boîte et autres objets présents.
...
Ne jamais essayer de faire entrer une scène entière dans une image. Une action comportant cause puis conséquence doit généralement être séparée.

Exemple A1 :

- Léa quitte le vrai chemin vers le pont ;
- Léa est engagée sur le pont ;
- le poids de Léa tend le cordage ;
- le mousqueton frappe la roche : CLAC ;
- Léa termine sa traversée ;
- Thomas regarde le pont puis Léa ;
- Léa se retourne de l'autre côté.

Ces instants peuvent nécessiter plusieurs cases précisément parce que le script demande au spectateur de voir certaines informations sans les surligner.
...
## Règle des indices ordinaires

### Mousqueton

À A1, il doit être clairement visible au moment du CLAC mais ne doit pas être traité comme une révélation. À A9, le contraste accroché/décroché doit être perceptible sans commentaire des personnages.
...
Sa chute normale doit d'abord être un événement banal. Sa fonction de relecture appartient à la partie inversée.

### Feuille

Même principe : événement naturel banal avant sa relecture.

### Caverne

Avant A15, son entrée ne doit pas être lisible comme une ouverture depuis les angles d'Éva et Léa. À A15, le changement d'angle doit réellement expliquer pourquoi elle devient visible.
...
Lorsque Thomas normal et Thomas inversé coexistent, ils sont le même personnage physique dans le même décor.
...
Le mouvement du mousqueton est une contrainte de continuité stricte.

Dans le temps propre de Thomas inversé : **DÉCROCHÉ → ACCROCHÉ**.

Dans le temps objectif normal, le même événement retourné doit correspondre à : **ACCROCHÉ → DÉCROCHÉ**.
...
Toutes les cases utilisent une géographie unique et stable :

- vrai chemin sur le flanc ;
- pont annexe plus bas dans le creux ;
- chemin rocheux plus long contournant le creux ;
- zone rocheuse fermée près du vrai chemin ;
- paroi ;
- précipice ;
- entrée de caverne masquée par un décrochement.

Avant une série de cases où la géographie est causalement importante, le cadrage doit être comparé à la bible de continuité.

## Règle de la lumière

La lumière suit l'heure objective de la scène, pas le sens psychologique de Thomas.

Le temps inversé n'est pas signalé par une palette bleue, rouge ou fantastique inventée.
...
- les mots ne sont pas réécrits ;
- le sens n'est pas paraphrasé ;
- aucune réplique explicative n'est ajoutée ;
- l'ordre des répliques reste celui du script.

Les bulles sont ajoutées après génération afin d'éviter les erreurs typographiques de génération d'image.

## Règle de validation

Après génération, chaque case reçoit un statut :

- `À VALIDER`
- `VALIDÉE`
- `À REFAIRE`

Une case `VALIDÉE` devient une référence visuelle obligatoire pour les suivantes.

Une case suivante ne doit pas modifier silencieusement : visage, coiffure, vêtements, sac, montre, taille relative, météo, lumière, géographie ou état d'un objet déjà verrouillé.

## Nommage des fichiers

Les images définitives seront numérotées dans l'ordre de lecture :
...
Les fiches pourront être conservées dans un registre unique avec le même identifiant.

## Assemblage final

Aucun PDF final n'est assemblé avant demande explicite de l'utilisateur.

Quand toutes les cases nécessaires sont validées :

1. définir les planches à partir des cases réellement produites ;
