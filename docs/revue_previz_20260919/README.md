...
## Passe de jeu F01/F02 publiée après accord utilisateur — 19/09/2026

Le générateur porte maintenant de brefs changements d'orientation des membres de la famille au départ et pendant l'attente sur A11, plus un regard de Thomas vers Éva partagé par A2/B9 selon l'instant objectif. Une petite boîte de préviz est brièvement montrée au niveau de la poche de Thomas pendant la pause de conclusion B9, et n'est pas révélée pendant A2. Il s'agit d'un proxy, non d'une animation articulée main–boîte ; vérifier son placement et sa visibilité au rendu. F02 garde tous ses chemins/angles de pont : observer que les regards n'altèrent pas la lisibilité des itinéraires. Voir F01 et F02 pour les contrôles ciblés.

**Non validé dans Unreal.** Pour tester les nouveaux accessoires et rebaker le casting, faire un run complet avec `FAST_CAMERA_ONLY=False`, `SUBTITLE_REVIEW_MODE=False`. Ne pas confondre un commit GitHub avec une séquence générée/visionnée. F07 demeure en validation distincte ; F03 reste en pause. La durée totale, les 17 pauses et les trajets objectifs ne sont pas remontés. **Ordre souhaité après validation : scène de la grotte (à réouvrir explicitement pour F03 si nécessaire), puis accélération/coupes des moments où rien ne se passe ; viser une vidéo courte, sans multiplier les effets gratuits.**


## Passe ciblée de correction A15 + F08 (19/09/2026 ; attente de nouveau rendu)

L'utilisateur demande de corriger d'abord la trajectoire visible de la caméra dans la grotte **et l'absence de sous-titres**. La validation A–Z, F01/F02/F07, l'animation fine et le rythme restent reportés. **F03 est réouverte pour cette correction ciblée ; non validée.** Dans le générateur, A15 a reçu des jalons de contournement/entrée et des diagnostics de dégagement 3D identifiant un potentiel obstacle. La géométrie et les rochers qui masquent l'entrée à Éva et Léa restent inchangés. Les 372 s, 17 pauses, chemins A/B, pont et convergence F07 sont conservés.

Pour F08, vérifier `POLOP_F03_GEOMETRY_ONLY` : quand ce mode vaut `1`, les sous-titres sont volontairement **absents** de toute la séquence. Le run complet vérifie désormais le nombre de sections sous-titres réellement créées et produit un `.srt` synchronisé avec le **film intégral**, en plus de la piste native Unreal. Ce fichier n'est pas automatiquement incrusté à la vidéo et ne peut pas être appliqué sans recalage à une capture partielle/accélérée. Un run dans Unreal et une lecture/capture contrôlée sont requis pour voir si la piste UMG s'affiche effectivement.

**Test de la nouvelle version :** `FAST_CAMERA_ONLY=False`, `SUBTITLE_REVIEW_MODE=False` et variable `POLOP_F03_GEOMETRY_ONLY` effacée ; lancement froid depuis une carte sauvegardée, journal `complete` et `omniscient_edit.json.subtitle_audit`. Contrôler A12 → A15 → A17 → B1 en lecture continue, plus texte d'introduction frame 0–90 et cartons de révélation/contact. Si un `F03 A15 ...` arrête la génération, relever l'acteur indiqué et la progression du **même run** avant toute autre retouche. Ni la grotte ni les sous-titres ne sont déclarés validés par les commits.


## Régression corrigée après revue de l'historique Git (19/09/2026)

**Le diagnostic précédent attribuant l'absence de sous-titres à un réglage de capture n'était pas établi.** La revue des commits a identifié une erreur de code reproductible par inspection : le commit historique `66b40fda` avait déjà validé la convention de `MovieSceneVisibilityTrack` (**True = visible** dans la piste Sequencer) et le fait que `MovieSceneBoolChannel.add_key` n'accepte **pas** `interpolation=`. Les nouvelles pistes de visibilité F01 (boîte) et F04 (anneau) utilisaient à nouveau l'inversion et cet argument interdit. La construction du film pouvait s'interrompre au premier `add_f01_box_to_film`, avant l'ajout des sous-titres, laissant une séquence partielle/non publiable. Le correctif `9a0f6d49` supprime l'argument sur toutes les clés booléennes des deux accessoires et restaure la convention validée. **La piste de visibilité des mannequins n'a pas été inversée ni modifiée.**

Le commit historique `2377e2cf` avait aussi introduit `POLOP_F03_GEOMETRY_ONLY` pour tester la grotte en sautant volontairement les sous-titres ; des commits F01/F04 ultérieurs s'en servaient également pour ne pas créer la boîte/l'anneau. Ce raccourci est désormais retiré dans `f2ce89e2` : `F03_GEOMETRY_ONLY=False` dans le générateur, **quelle que soit** la variable héritée du processus Unreal. La régénération normale conserve systématiquement les sous-titres, le casting et les accessoires. `FAST_CAMERA_ONLY` reste à `False` par défaut ; ne pas l'activer pour un test des animations/sous-titres.

**Contrôle statique post-correctif :** 372 s, 17 pauses ; les trois familles de clés booléennes (mannequins, boîte, anneau) utilisent `add_key(frame, bool)` sans `interpolation=`. **Aucun run Unreal ni contrôle visuel n'a été exécuté depuis ces correctifs.** Après synchronisation, lancer un run complet depuis la carte enregistrée, relever le premier `RuntimeError` éventuel du `keylog.jsonl` et vérifier les mannequins articulés, la boîte B9, l'anneau A15/A17 et les sous-titres dans le même film. Un blocage `F03 A15 ...` serait un problème de contrôle de caméra distinct, à résoudre sans retirer les pistes de casting/sous-titres.
