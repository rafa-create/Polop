...
root = os.path.join(saved,"POLOP","V11")
png_path = os.path.join(root,"polop_v11_heightmap_1009.png")
model_path = os.path.join(root,"route_model_v11.json")

if not os.path.exists(png_path):
    raise RuntimeError("Heightmap V11 absent")
if not os.path.exists(model_path):
    raise RuntimeError("route_model_v11 absent")
...
def load_png16(path):
    ...
    if whdc != (1009,1009,16,0):
        raise RuntimeError("PNG V11 inattendu : "+str(whdc))
...
report = {
    ...
    "timing":model["timing"],
    "visual_review_required": ["03_BRIDGE_A","06_CAVE_HUMAN","07_HUMAN_A","08_HUMAN_B"],
}

outdir = os.path.join(root,"validation")
os.makedirs(outdir,exist_ok=True)
json_path = os.path.join(outdir,"validation_v11.json")
txt_path = os.path.join(outdir,"validation_v11.txt")
