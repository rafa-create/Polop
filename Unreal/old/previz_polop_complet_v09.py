...
# =============================================================================
# LA BOUCLE / POLOP — PREVIZ COMPLETE V09
# SENTIERS COURBES + PONT + GROTTE + PROXIES HUMAINS
# Unreal Engine 5.8 — 1 uu = 1 cm
#
# V08 est validée techniquement. V09 NE CHANGE PAS la topologie :
...
#
# V09 améliore uniquement la forme :
...
#
# Après exécution :
#   Saved/POLOP/V09/polop_v09_heightmap_1009.png
#   Saved/POLOP/V09/route_model_v09.json
#
# IMPORT DU LANDSCAPE V09 :
...
unreal.log("POLOP V09 : calcul du relief de base...")
...
unreal.log("POLOP V09 : sculpture des sentiers courbes...")
...
out_dir = os.path.join(saved, "POLOP", "V09")
os.makedirs(out_dir, exist_ok=True)

heightmap_path = os.path.join(out_dir, "polop_v09_heightmap_1009.png")
route_path = os.path.join(out_dir, "route_model_v09.json")
instructions_path = os.path.join(out_dir, "IMPORT_V09.txt")

unreal.log("POLOP V09 : écriture du PNG...")
...
with open(instructions_path, "w", encoding="utf-8") as f:
    f.write(
        ...
        "Puis exécuter previz_polop_validation_v09.py.\n" % heightmap_path
    )
...
MAT_A = ensure_material("M_V09_A", (0.05,0.35,1.0))
MAT_B = ensure_material("M_V09_B", (1.0,0.18,0.04))
MAT_UP = ensure_material("M_V09_UP", (0.5,0.18,0.95))
MAT_CAVE = ensure_material("M_V09_CAVE_ACCESS", (0.85,0.85,0.85))
MAT_FLANK = ensure_material("M_V09_FLANK", (0.05,0.78,0.15))
MAT_BRIDGE = ensure_material("M_V09_BRIDGE", (0.95,0.68,0.03))
MAT_CAVE_DARK = ensure_material("M_V09_CAVE_DARK", (0.04,0.04,0.05))
MAT_MARKER = ensure_material("M_V09_MARKER", (1.0,1.0,0.03))
MAT_THOMAS = ensure_material("M_V09_THOMAS", (0.08,0.35,1.0))
MAT_INVERSE = ensure_material("M_V09_INVERSE", (1.0,0.12,0.05))
MAT_EVA = ensure_material("M_V09_EVA", (0.75,0.15,0.85))
MAT_LEA = ensure_material("M_V09_LEA", (0.03,0.82,0.68))
...
def cave_blockout():
    ...
    z = height_at(p_end[0],p_end[1])
    tunnel_length_m = 12.0
    half_width_m = 3.0
    wall_t_m = 1.2
    wall_h_m = 4.8
    ...
    spawn_box(
        "GROTTE_TOIT",
        V(cx*100,cy*100,(z+wall_h_m+0.45)*100),
        (tunnel_length_m*100,(half_width_m*2+wall_t_m*2)*100,90.0),
        MAT_CAVE_DARK,
        "Props/Grotte",
        rot
    )

    back_x = p_end[0] + ux*tunnel_length_m
    back_y = p_end[1] + uy*tunnel_length_m
    spawn_box(
        "GROTTE_FOND",
        V(back_x*100,back_y*100,(z+wall_h_m/2.0)*100),
        (100.0,(half_width_m*2+wall_t_m*2)*100,wall_h_m*100),
        MAT_CAVE_DARK,
        "Props/Grotte",
        rot
    )
...
def make_camera(name,pos,target,focal):
    try:
        ...
        label(cam,"CAM_REVIEW_"+name,"Cameras/Review_V09")
        ...
        unreal.log("POLOP V09 CAM OK : "+name)
        return cam
    except Exception as exc:
        unreal.log_error("POLOP V09 CAM FAIL %s : %s"%(name,exc))
        return None
...
camera_specs = [
    ...
    ("02_FLANC_CLOSE", V(74000,-12000,15000), target(825,45,900), 45),
    ("03_BRIDGE_A", V(76000,-19000,18000), target(900,12.5,750), 42),
    ("04_BRIDGE_B", V(102000,23000,19000), target(900,12.5,750), 42),
    ("05_HIGH_JUNCTION", V(157000,-27000,30000), target(1770,25,1200), 45),
    ("06_CAVE_ACCESS", V(170000,-9000,22000), target(1830,90,900), 45),
    ("07_HUMAN_A", V(62000,-8000,height_at(620,-80)*100+175), target(720,-50,165), 35),
    ("08_HUMAN_B", V(131000,59500,height_at(1310,595)*100+175), target(1190,520,165), 35),
    ("09_CONVERGENCE", V(-7000,-9000,height_at(0,0)*100+5000), target(35,3,900), 50),
]
...
unreal.log("============================================================")
unreal.log("POLOP PREVIZ V09 PREPAREE")
